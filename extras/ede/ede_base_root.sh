...
YUM_PACKAGES="gcc gcc-c++ autoconf automake libtool subversion subversion-perl rpm-build httpd httpd-devel openssl-devel jpackage-utils pcre-devel expat-devel unixODBC-devel junit ant-commons-logging postgresql-server zlib-devel postgresql-devel alsa-lib-devel gnutls-devel mysql-devel ncurses-devel python-devel ruby ruby-devel ruby-rdoc bind tftp-server doxygen zip which unzip createrepo ant-junit mod_ssl libXp libpng-devel libart_lgpl-devel freetype freetype-devel gdb gdbm-devel mysql-devel ncurses-devel vsftpd mod_perl-devel dhcp net-snmp-utils ntp yum-utils java-1.6.0-openjdk java-1.6.0-openjdk-devel redhat-rpm-config ant ant-trax ant-nodeps jakarta-commons-collections jakarta-commons-beanutils log4j mrtg stunnel"
