    ...
    {
      {
        ...
        if (iteration++ >= 10)
        {
          ...
          {
            OsSysLog::add(FAC_KERNEL, PRI_DEBUG, "OsTimer::TimerService timer resolution: %d microseconds.", skew);
          }
          else
          {
           OsSysLog::add(FAC_KERNEL, PRI_WARNING, "OsTimer::TimerService timer resolution: %d microseconds.", skew);
          }
          iteration = 0;
        }
...
bool OsTimer::Timer::oneshotAt(const OsDateTime& t)
{
  ...
  {
    OsSysLog::add(FAC_KERNEL, PRI_ERR, "OsTimer::Timer::oneshotAt timer expiration is in the past.  Call ignored.");
    return false;
  }
