   ...
   case SipSubscribeClient::SUBSCRIPTION_SETUP:
   {
      ...
      else
      {
         OsSysLog::add(FAC_RLS, PRI_ERR,
                       "SubscriptionSet::addInstance cannot add reg subscription with dialog handle '%s', already %d in ContactSet '%s'",
                   ...
                   !uri_allocated->isNull())
               {
                  ...
                  if (strcmp(state, "active") == 0)
                  {
                     ...
                     if (!state_from_this_subscr->find(id_allocated))
                     {
                        ...
                        {
                           OsSysLog::add(FAC_RLS, PRI_ERR,
                                         "SubscriptionSet::notifyEventCallback cannot add Call-Id;RUI '%s', already %d in ContactSet '%s' subscription '%s'",
                                         uri_allocated->data(),
