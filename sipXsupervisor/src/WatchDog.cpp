...
UtlString WatchDog::getServiceVersion(const UtlString &service)
{
   UtlString result = "";
   ...
   //TEMP TEMP
   //When its in place, we'll need to read the new process xml file for the service in question and extract the version
   //string from it.
   //TEMP TEMP
   OsProcess process;
   OsStatus rc = OsProcessMgr::getInstance()->getProcessByAlias(service, process);
   if (OS_SUCCESS == rc)
   {
      OsProcessInfo ProcessInfo;
      rc = process.getInfo( ProcessInfo);
      if (OS_SUCCESS == rc)
      {
...
UtlBoolean WatchDog::setServiceVersion(const UtlString &service, const UtlString &version)
{
   ...
   //TEMP TEMP
   //When its in place, we'll need to read the new service xml file for the service in question and replace the version
   //string from it and write it back.
   //TEMP TEMP
   OsProcess process;
   OsStatus rc = OsProcessMgr::getInstance()->getProcessByAlias(service, process);
   if (OS_SUCCESS == rc)
