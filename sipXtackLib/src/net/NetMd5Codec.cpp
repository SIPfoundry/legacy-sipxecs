...
void NetMd5Codec::appendHashValue(UtlString& output)
{
   ...
   char szTmp[ENCODED_SIZE + 1];
   for (int i = 0; i < 16; i++)
   {
      sprintf(szTmp+i*2, "%02x", mDigest[i]);
   }
   output.append(szTmp);
}
...
/// copied from MD5C.C - RSA Data Security, Inc., MD5 message-digest algorithm
...
static void Encode PROTO_LIST
  ((unsigned char *, const UINT4 *, size_t));
static void Decode PROTO_LIST
  ((UINT4 *, const unsigned char *, size_t));
 ...
 */
void MD5Init (MD5_CTX_PT *context) /* context */
{
