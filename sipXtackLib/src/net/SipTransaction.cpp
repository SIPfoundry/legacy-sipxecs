...
#          endif
             ...
             )
    {
        ...
        sendSucceeded = recurseDnsSrvChildren(userAgent, transactionList, reevaluateDestination, requestCopy);
    }
                                                 ...
                                                 SipMessage* pRequest)
{
        ...
        mChildTransactions.isEmpty())
    {
        ...
        else if(mTransactionState < TRANSACTION_CONFIRMED)
        {
            ...
            {
              UtlString routeUri;
              if (!pRequest->getRouteUri(0, &routeUri))
              {
              ...
              else
              {
                UtlString transport;
                Url routeUrlParser(routeUri);
                routeUrlParser.getUrlParameter("transport", transport);
                if (transport == "tls" || transport == "TLS")
                {
                  scheme = "sips";
                  msgSizeProtocol = OsSocket::SSL_SOCKET;
...
#               endif
                ...
                if(childTransaction->handleOutgoing(recursedRequest,
                                                 userAgent,
                                                 transactionList,
                                                 MESSAGE_REQUEST, reevaluateDestination))
                {
                    childRecursed = TRUE;
                }
                                   ...
                                   int& nextTimeout)
{
    ...
    if (protocol == OsSocket::UDP)
    {
        ...
        {
            // Try UDP again
            if (userAgent.sendUdp(&resendMessage, sendAddress.data(), sendPort, NULL))
            {
    ...
    if(relationship == MESSAGE_DUPLICATE)
    {
        ...
        if(!(incomingMessage.isResponse()))
        {
            ...
            #                    endif
                   ...
                   else if (method.compareTo(SIP_INVITE_METHOD) != 0 && !SipTransaction::SendTryingForNist)
                   {
                     //
                     // Set the transaction state to CALLING without sending any response
                     //
                      if (mTransactionState < TRANSACTION_CALLING)
                      {
                          mTransactionState = TRANSACTION_CALLING;
