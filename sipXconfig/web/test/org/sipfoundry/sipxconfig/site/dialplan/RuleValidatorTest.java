...
public class RuleValidatorTest extends TestCase {
    ...
    public void testValidateEnabledEmergencyRuleWithCallerSensitiveForwarding() {
        RuleValidator out = new RuleValidator();
        EmergencyRule rule = new EmergencyRule();
        rule.setEnabled(true);
        rule.setUseMediaServer(true);
        try {
            // enabled emergency rules with caller sensitive forwarding are valid
            out.validate(null, null, rule);
        } catch (ValidatorException ve) {
            fail(ve.getMessage());
        }
