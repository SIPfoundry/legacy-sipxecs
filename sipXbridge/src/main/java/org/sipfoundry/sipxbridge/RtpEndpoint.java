...
public class RtpEndpoint {
    private static Logger logger = Logger.getLogger(RtpEndpoint.class);
    ...
    class EarlyMediaSender extends TimerTask {
        ...
        EarlyMediaSender() {
            ...
            logger
                    .debug("Starting early media sender TimerTask : remote endpoint = "
                            + RtpEndpoint.this.getIpAddress()
                            + "/ "
                            + RtpEndpoint.this.port);
            for (int i = 0; i < 32; i++) {
                data[i] = (byte) 0x7F;
            }
        ...
        public void run() {
            try {
                 ...
                 */
                rtpPacket.setPayload(data, data.length);
                ...
                ByteBuffer packed = rtpPacket.getData();
                send(packed);
     ...
     */
    public RtpEndpoint(boolean isTransmitter, boolean useGlobalAddress)
            throws IOException {
        ...
        if (!isTransmitter) {
            ...
            if (!useGlobalAddress)
                ...
                this.ipAddress = Gateway.getGlobalAddress();
            ...
            for (this.port = Gateway.getRtpPortRangeLowerBound(); port < Gateway.getRtpPortRangeUpperBound(); port += 2) {
    ...
    void startEarlyMediaThead() {
        ...
        Gateway.timer.schedule(this.earlyMediaSender, 0, 160);

    }

    public void stopEarlyMediaThread() {
        if (!this.isTransmitter)
            throw new IllegalStateException(
                    "Operation only permitted on transmitters");
        if (!this.earlyMediaStarted)
            return;
        logger.debug("stopEarlyMediaThread");
        this.earlyMediaStarted = false;
        if (this.earlyMediaSender != null) {
            this.earlyMediaSender.cancel();
            this.earlyMediaSender = null;
     ...
     */
    public boolean isEarlyMediaStarted() {
