...
public class BackToBackUserAgent {
		...
		try {
			...
			if (Gateway.getAccountManager().getBridgeConfiguration()
				...
				if (itspAccountInfo == null
						|| accountInfo.isGlobalAddressingUsed()) {
		...
		try {
			...
			if (dialog == null || dialog.getState() == DialogState.TERMINATED) {
				...
				Response response = SipUtilities.createResponse(
						...
						referRequest, Response.NOT_ACCEPTABLE);
			...
			if (stx.getState() != TransactionState.TERMINATED) {
				...
				stx.sendResponse(response);
				/*
				 * This flag controls whether or not we forward the BYE 
				 * when the refer agent tears down his end of the dialog.
				 */
				...
				/*
				 * This case should not occur. This can happen only if the
				 * transaction is timed out or canceled or if the response
				 * arrived late.
				 */
				logger
						.warn("referInviteToSipxProxy on a terminated transaction");
				return;
			}
			 ...
			 */
			DialogApplicationData.get(ct.getDialog()).setPendingAction(PendingDialogAction.PENDING_RE_INVITE_WITH_SDP_OFFER);
	 ...
	 */
	boolean solicitSdpOfferFromPeerDialog(RequestEvent requestEvent,
	...
	void sendInviteToItsp(RequestEvent requestEvent,
		...
		try {
			...
			if (spiral) {
				outboundSessionDescription = DialogApplicationData
		...
		try {
			...
			if (Gateway.isReInviteSupported()
				...
				if (replacedDialog.getState() != DialogState.CONFIRMED) {
					...
					SipUtilities.cleanSessionDescription(newOffer,
							selectedCodec);
	...
	void processBye(RequestEvent requestEvent) throws SipException {
		...
		if (dialogContext.forwardByeToPeer && peer != null
			...
			TransactionApplicationData transactionContext = TransactionApplicationData.attach(
