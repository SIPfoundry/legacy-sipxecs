	...
	void sendByeToMohServer() {
		try {
			...
			if (this.musicOnHoldDialog != null
				...
				DialogContext dialogCtx = DialogContext
						.get(this.musicOnHoldDialog);
	...
	void handleInviteWithReplaces(RequestEvent requestEvent,
		...
		try {
			...
			if (replacedDialog.getState() == DialogState.CONFIRMED) {
				...
				okResponse.setContent(rtpSession.getReceiver().getSessionDescription().toString(), cth);
				...
				SessionDescription answerSdes = SipUtilities.cleanSessionDescription(newOffer, codecs);
