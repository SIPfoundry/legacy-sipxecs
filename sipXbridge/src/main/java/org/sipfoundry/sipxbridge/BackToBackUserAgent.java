...
public class BackToBackUserAgent {
	...
	private RtpSession getLanRtpSession(Dialog dialog) throws IOException {
		try {
			...
			if (dialogApplicationData.rtpSession == null) {
				...
				RtpEndpoint endpoint = new RtpEndpoint(false);
				endpoint.setIpAddress(Gateway.getLocalAddress());
				rtpSession.setMyEndpoint(endpoint);
				this.getRtpBridge().addRtpSession(rtpSession);
				...
				this.rtpBridge.addRtpSession(rtpSession);
    ...
    public RtpSession getWanRtpSession(Dialog dialog) {
        try {
            ...
            if (rtpSession == null) {
                 ...
                 *Allocate a receiver. 
                 */
                RtpEndpoint mediaEndpoint = new RtpEndpoint(false);
                 mediaEndpoint.setIpAddress(itspAccountInfo.isGlobalAddressingUsed()? Gateway.getGlobalAddress() :
	...
	private void handleSpriralInviteWithReplaces(RequestEvent requestEvent,
		...
		try {
			...
			Set<RtpSession> myrtpSessions = this.getRtpBridge()
					.getRtpSessionTable();
			...
			DialogApplicationData replacedDialogApplicationData = (DialogApplicationData) replacedDialog
					.getApplicationData();
			RtpBridge hisRtpBridge = replacedDialogApplicationData.backToBackUserAgent.getRtpBridge();
			hisRtpBridge.pause();
			Set<RtpSession> hisRtpSessions = hisRtpBridge.getRtpSessionTable();
			
			Set<RtpSession> mergedRtpSessions = new HashSet<RtpSession>();
			mergedRtpSessions.addAll(myrtpSessions);
			mergedRtpSessions.addAll(hisRtpSessions);
			mergedRtpSessions.remove(DialogApplicationData.getRtpSession(replacedDialog));
			mergedRtpSessions.remove(DialogApplicationData.getRtpSession(referingDialog));
			...
			for (RtpSession rtpSession : mergedRtpSessions) {
			...
			hisRtpBridge.resume();
			RtpBridge newRtpBridge = new RtpBridge(mergedRtpSessions);
			newRtpBridge.start();
	...
	public void referInviteToSipxProxy(Request referRequest, Dialog dialog) {
		...
		try {
			...
			newRequest.setContent(lanRtpSession.getReceiver().getSessionDescription().toString(), cth);
			/*
			...
			DialogApplicationData newDialogApplicationData = DialogApplicationData.attach(this,ct.getDialog());
	...
	public void sendInviteToSipxProxy(RequestEvent requestEvent,
		...
		try {
			...
			RtpSession incomingSession = this.getWanRtpSession(
					inboundDialog);
			...
			newRequest.setContent(this.getLanRtpSession(outboundDialog).getReceiver().getSessionDescription()
					.toString(), cth);
			...
			tad.incomingSession = this.getWanRtpSession(
					inboundDialog);
			tad.outgoingSession = this.getLanRtpSession(
					outboundDialog);
	...
	public void sendInviteToItsp(RequestEvent requestEvent,
		...
		try {
			...
			SessionDescription sd = spiral ? DialogApplicationData.getRtpSession(
					...
					: this.getWanRtpSession(outboundDialog).getReceiver()
			...
			if (!spiral) {
				...
				RtpEndpoint rtpEndpoint = new RtpEndpoint(true);
				//rtpEndpoint.setMaxSilence(Gateway.getMediaKeepaliveMilisec());
				this.getLanRtpSession(incomingDialog).setRemoteEndpoint(rtpEndpoint);
				rtpEndpoint.setSessionDescription(sessionDescription);
