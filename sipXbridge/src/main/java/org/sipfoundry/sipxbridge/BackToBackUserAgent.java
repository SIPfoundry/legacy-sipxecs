...
public class BackToBackUserAgent {
		...
		try {
			...
			if (rtpSession == null) {
				...
				String ipAddress = (itspAccountInfo == null || itspAccountInfo
						.isGlobalAddressingUsed()) ? symmitronClient
						.getPublicAddress() : symmitronClient
						.getServerAddress();
	...
	void handleSpriralInviteWithReplaces(RequestEvent requestEvent,
			...
			String toDomain, boolean isphone) throws SipException {
		/* The inbound INVITE */
	...
	BackToBackUserAgent(SipProvider provider, Request request, Dialog dialog,
		...
		/*
		 * Received this reuqest from the LAN The symmitron to use is the
		 * symmitron that runs where the request originated from.
		 */
	 ...
	 * This is a garbage collection routine. When all the dialogs
	 * referencing this structure have terminated, then we send BYE
	...
	void removeDialog(Dialog dialog) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Remove Dialog " + dialog  + " Dialog table size = " + this.dialogTable.size());
		}
	 ...
	 * This implies that the given dialog holds a pointer to this structure.
	 * 
	 * @param provider
	 * @param dialog
	 */
	...
	Request createInviteFromReferRequest(RequestEvent requestEvent)
		...
		/*
		 * Get the Refer-To header and convert it into an INVITE to send to the
		 * REFER target.
		 */
		...
		for (Iterator it = uri.getHeaderNames(); it.hasNext();) {
			...
			if (headerValue != null) {
				...
				String decodedHeaderValue = URLDecoder.decode(headerValue,
						"UTF-8");
				header = (Header) ProtocolObjects.headerFactory.createHeader(
						headerName, decodedHeaderValue);
			}
			if (header != null) {
		...
		FromHeader fromHeader = (FromHeader) newRequest.getHeader(
	 ...
	 * redirect the INVITE to the contact mentioned in the Refer. To determine
	 * the codec that was negotiated in the original Call Setup, we send an
	 * INVITE (no-sdp) to the dialog to solicit an offer. This operation has
	 * already returned a result when this method is called.
	 * 
	 * 
	 * @param inviteRequest
	 *            -- the INVITE request.
	...
	void referInviteToSipxProxy(Request inviteRequest,
			RequestEvent referRequestEvent,
		...
		try {
			...
			if (stx.getState() != TransactionState.TERMINATED) {
				Response response = ProtocolObjects.messageFactory
						.createResponse(Response.ACCEPTED, referRequest);
			...
			DialogApplicationData newDialogApplicationData = DialogApplicationData
					.attach(this, ct.getDialog(), ct, ct.getRequest());
			...
			TransactionApplicationData tad = TransactionApplicationData.attach(
					ct, Operation.REFER_INVITE_TO_SIPX_PROXY);
			 ...
			 * this in the INVITE processing below. SIPX will not strip any 
	...
	void forwardReferToItsp(RequestEvent requestEvent) {
		...
		try {
			...
			ItspAccountInfo itspInfo = DialogApplicationData.get(peerDialog)
					.getItspInfo();
			...
			tad.serverTransactionProvider = (SipProvider) requestEvent
					.getSource();
	...
	void sendInviteToSipxProxy(RequestEvent requestEvent,
		...
		try {
			...
			DialogApplicationData.attach(this, outboundDialog, ct, ct
					.getRequest());
			/*
			 * Set the ITSP account info for the inbound INVITE to sipx proxy.
			 */
			DialogApplicationData.get(outboundDialog).setItspInfo(
					itspAccountInfo);
			...
			this.getLanRtpSession(outboundDialog).getReceiver()
	...
	ClientTransaction createClientTxToMohServer(
		...
		try {
			...
			TransactionApplicationData tad = TransactionApplicationData.attach(
					ct, Operation.SEND_INVITE_TO_MOH_SERVER);
			...
			tad.backToBackUa = this;
			this.addDialog(ct.getDialog());
		    DialogApplicationData.attach(this, ct
	...
	void sendByeToMohServer() {
		try {
			if (this.musicOnHoldDialog != null
				...
				ClientTransaction ctx = lanProvider
	 ...
	 * @throws Exception
	 */
	boolean querySdpFromPeerDialog(RequestEvent requestEvent,
			Operation continuation, ContinuationData continuationData)
			throws Exception {
		try {
			Dialog dialog = requestEvent.getDialog();
			Dialog peerDialog = DialogApplicationData.getPeerDialog(dialog);
			...
			if (peerDialog != null
				...
				TransactionApplicationData tad = TransactionApplicationData
						.attach(ctx, Operation.QUERY_SDP_FROM_PEER_DIALOG);
	...
	void sendInviteToItsp(RequestEvent requestEvent,
		...
		try {
			...
			String address = itspAccountInfo.isGlobalAddressingUsed() ? this.symmitronClient
					.getPublicAddress()
					: this.symmitronClient.getServerAddress();
			...
			TransactionApplicationData tad = new TransactionApplicationData(
					serverTransaction, Operation.SEND_INVITE_TO_ITSP);
		...
		try {
			...
			if (Gateway.isReInviteSupported()
				...
				ClientTransaction ctx = provider
						.getNewClientTransaction(reInvite);
				TransactionApplicationData tad = TransactionApplicationData
						.attach(ctx, Operation.HANDLE_INVITE_WITH_REPLACES);
				tad.setServerTransaction(serverTransaction);
				...
				/*
				 * Re-INVITE is not supported. Remap the transmitter side and
				 * hang up the replaced dialog.
				 */
				...
				if (replacedDialog.getState() != DialogState.CONFIRMED) {
					...
					selectedCodec = RtpPayloadTypes.getPayloadType(codecs
							.iterator().next());
				...
				if (replacedDialog.getState() != DialogState.CONFIRMED) {
					/*
					 * The other side has not been sent an SDP answer yet.
					 * Extract the OFFER from the inbound INVITE and send it as
					 * an answer.
					 */
					DialogApplicationData replacedDialogApplicationData = DialogApplicationData
							.get(dialog);
					...
					Response peerOk = ProtocolObjects.messageFactory
							.createResponse(Response.OK, peerSt.getRequest());
			...
			try {
				this
						.tearDown(ProtocolObjects.headerFactory
								.createWarningHeader("sipxbridge", 103, ex
										.getMessage()));
			} catch (Exception e) {
				logger.error("Unexpected exception ", e);
			}
	...
	void processBye(RequestEvent requestEvent) throws SipException {
		...
		if ((!dad.isReferAccepted) && peer != null
	...
	/**
	 * Terminate the two sides of the bridge. This method is invoked by xml rpc
	 * interface and is hence public.
	 */
	...
	public SessionDescription getTransmitterSd(Dialog dialog)
			throws IOException {
		SessionDescription transmitterSd = ((DialogExt) dialog)
				...
				.getSessionDescription() : this.getWanRtpSession(dialog)
