...
class CallControlManager implements SymmitronResetHandler {
	...
	class RequestPendingTimerTask extends TimerTask {
		...
		public void run() {
			try {
				Request request = transactionContext.getClientTransaction()
						.getRequest();
				DialogContext dialogContext = DialogContext
						.get(transactionContext.getClientTransaction()
								.getDialog());
				Dialog dialog = transactionContext.getClientTransaction()
						.getDialog();
				...
				try {
					BackToBackUserAgent b2bua = DialogContext
							.getBackToBackUserAgent(transactionContext
									.getClientTransaction().getDialog());
					logger.error("Error sending re-INVITE", ex);
					b2bua.tearDown("sipxbridge",
							ReasonCode.ERROR_SENDING_REINVITE,
	...
	class NotifyReferDialogTimerTask extends TimerTask {
		...
		public NotifyReferDialogTimerTask(Request referRequest,
				Dialog referDialog, Response response) {
		...
		try {
			...
			if ((DialogContext) dialog.getApplicationData() != null) {
				...
				if (btobua == null) {
					...
					response
							.setReasonPhrase("No record of ITSP. Check configuration.");
					serverTransaction.sendResponse(response);
					return;
				}
			...
			if (provider == Gateway.getLanProvider()) {
				...
				if (account.getState() == AccountState.INVALID) {
					Response response = SipUtilities.createResponse(
							serverTransaction, Response.BAD_GATEWAY);
					response
	...
	private void processPrack(RequestEvent requestEvent) {
		try {
			 ...
			 */
			Response prackOk = SipUtilities.createResponse(serverTransactionId,
					Response.OK);
	...
	/*
	 * This is a polycomm specific hack. Polycomm sometimes sends 500 if NOTIFY
	 * sent too early.
	 */

	private void delayNotifyReferDialog(Request referRequest,
			Dialog referDialog, Response response) throws SipException {

		if (response.getStatusCode() == Response.OK) {
			Gateway.getTimer().schedule(
					new NotifyReferDialogTimerTask(referRequest, referDialog,
							response), 2000);
		} else {
			this.notifyReferDialog(referRequest, referDialog, response);
		}
	...
	private void inviteToItspOrProxyResponse(ResponseEvent responseEvent)
		 ...
		 * If he sends a BYE do not forward it. Set a Flag indicating BYE should
		 * not be forwarded.
		 */
	...
	private void handleInviteWithReplacesResponse(ResponseEvent responseEvent)
		...
		Response serverResponse = SipUtilities.createResponse(
				serverTransaction, response.getStatusCode());
			...
			if (response.getContentLength().getContentLength() != 0) {
				...
				if (peerProvider == Gateway.getLanProvider()) {
					rtpSession.getReceiver().setUseGlobalAddressing(false);
						...
						|| DialogContext.get(peerDialog).getItspInfo()
								.isGlobalAddressingUsed()) {
		...
		if (response.getRawContent() != null
			...
			if (rtpSession != null) {
				...
				if (tad.getServerTransaction() != null
					...
					Response forwardedResponse = SipUtilities.createResponse(
							tad.getServerTransaction(), response
									.getStatusCode());
					SipUtilities.setSessionDescription(forwardedResponse,
							sessionDescription);
					ContactHeader contact = SipUtilities.createContactHeader(
	 ...
	 * @throws Exception
	 */
	private void sendInviteToMohServerResponse(ResponseEvent responseEvent)
			throws Exception {
		Response response = responseEvent.getResponse();
		Dialog dialog = responseEvent.getDialog();
		TransactionContext tad = TransactionContext.get(responseEvent
		...
		if (response.getStatusCode() == Response.OK) {
			...
			if (tad.getDialogPendingSdpAnswer() != null
				...
				if (logger.isDebugEnabled()) {
					logger.debug("dialogPendingSdpAnswer = "
							+ tad.getDialogPendingSdpAnswer());
					if (tad.getDialogPendingSdpAnswer() != null) {
						logger.debug("PendingAction = "
								+ DialogContext.getPendingAction(tad
		...
		if (continuationOperation == Operation.REFER_INVITE_TO_SIPX_PROXY) {
			...
			if (dialog.getState() != DialogState.TERMINATED) {
				...
				if (!Gateway.getBridgeConfiguration()
					...
					SessionDescription sessionDescription = SipUtilities
							.getSessionDescription(response);
					Set<Integer> codecs = SipUtilities
							.getMediaFormats(sessionDescription);
			...
			if (!SipUtilities.isCodecSupported(responseSessionDescription,
				 ...
				 */
				Set<Integer> codecs = SipUtilities
						.getMediaFormats(responseSessionDescription);
				SipUtilities.cleanSessionDescription(ackSd, codecs);
	...
	private void processInviteResponse(ResponseEvent responseEvent) {
		...
		try {
			...
			if (response.getStatusCode() == Response.TRYING) {
				 ...
				 * the response is for. This is an OK for the session timer.
				 */
				...
				/*
				 * Note that the BAD_REQUEST check should not be necessary here
				 * Some ITSPs send a bad error code and hence this check for
				 * BAD_REQUEST.
				 */
				transactionContext.counter++;
				Gateway.getTimer().schedule(
						new RequestPendingTimerTask(transactionContext), 1000);
