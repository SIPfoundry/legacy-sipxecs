...
class CallControlManager implements SymmitronResetHandler {
	...
	private void handleReInvite(RequestEvent requestEvent) throws Exception {
		...
		if (SipUtilities.isSdpOfferSolicitation(request)) {
			...
			TransactionApplicationData tad = TransactionApplicationData.attach(
					ctx, Operation.FORWARD_SDP_SOLICITIATION);
			 ...
			 * response arrives.
			 */
			tad.continuationData = new ForwardSdpSolicitationContinuationData(
					requestEvent);
	...
	private void processAck(RequestEvent requestEvent) {
		try {
			...
			if (peerDialogContext != null
				...
				logger.debug("peerDialogAction " + peerDialogContext.getPendingAction());
				if (inboundAck.getContentLength().getContentLength() != 0) {
					...
					if (peerDialogContext.getPendingAction() == PendingDialogAction.PENDING_FORWARD_ACK_WITH_SDP_ANSWER
							&& dialogContext.mohCodecNegotiationFailed) {
						logger
						...
						SessionDescription sd = peerDialogContext
								.getRtpSession().getReceiver()
								.getSessionDescription();

						SipUtilities.setDuplexity(sd, "sendrecv");
						ack.setContent(sd.toString(), cth);
						dialogContext.mohCodecNegotiationFailed = false;

					} else if (peerDialogContext.getPendingAction() == PendingDialogAction.PENDING_FORWARD_ACK_WITH_SDP_ANSWER) {
						/*
						...
						/*
						 * The other side expects an SDP Answer and we dont have
						 * one.
						 */
						return;
					} else {
	...
	private void inviteToItspOrProxyResponse(ResponseEvent responseEvent)
		...
		if (response.getRawContent() != null
			...
			if (transactionContext.operation == Operation.SEND_INVITE_TO_ITSP) {
				keepaliveMethod = transactionContext.itspAccountInfo
	...
	private void forwardSdpSolicitationResponse(ResponseEvent responseEvent)
		...
		if (response.getContentLength().getContentLength() != 0) {
			...
			DialogApplicationData.getPeerRtpSession(dialog).getReceiver()
			...
			newResponse.setHeader(contactHeader);
			// Mark that we should forward the sdp answer in ACK.
			peerDialogContext
					.setPendingAction(PendingDialogAction.PENDING_FORWARD_ACK_WITH_SDP_ANSWER);
	...
	private void handleInviteWithReplacesResponse(ResponseEvent responseEvent)
		...
		if (response.getContentLength().getContentLength() != 0) {
			...
			RtpSession inboundRtpSession = DialogApplicationData
					.getRtpSession(dialog);
			inboundRtpSession.getTransmitter().setSessionDescription(
					sdesCloned, false);

			RtpSession outboundRtpSession = DialogApplicationData.get(
	...
	private void forwardReInviteResponse(ResponseEvent responseEvent)
		...
		if (serverTransaction != null) {
			...
			if (response.getContentLength().getContentLength() != 0) {
				...
				RtpSession originalRtpSession = DialogApplicationData
						.getRtpSession(dialog);
	...
	private void referInviteToProxyOrBlindTransferToItspResponse(
		...
		if (response.getContentLength().getContentLength() != 0) {
			if (tad.dialogPendingSdpAnswer != null
					&& DialogApplicationData.get(tad.dialogPendingSdpAnswer)
	...
	private void solicitSdpOfferFromPeerDialogResponse(
		...
		if (continuationOperation == Operation.REFER_INVITE_TO_SIPX_PROXY) {
			...
			if (dialog.getState() != DialogState.TERMINATED) {
				...
				if (!Gateway.getBridgeConfiguration()
					...
					DialogApplicationData.getRtpSession(
							continuation.getDialog()).getReceiver()
							.setSessionDescription(responseSessionDescription);
				...
				DialogApplicationData.getRtpSession(continuation.getDialog())
			...
			if (!SipUtilities.isCodecSupported(responseSessionDescription,
				...
				DialogApplicationData.get(continuation.getDialog()).mohCodecNegotiationFailed = true;
				...
				RtpSession lanRtpSession = DialogApplicationData
						.getPeerRtpSession(dialog);
				RtpSession wanRtpSession = DialogApplicationData
						.getRtpSession(dialog);
