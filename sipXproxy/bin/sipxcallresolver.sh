...
. /home/joegen/sipxecs-bin/libexec/sipXecs/sipx-utils.sh

# Declare script variables in a manner that unit test
# (or end user I suppose) can override
: ${ConfDir:=/home/joegen/sipxecs-bin/etc/sipxpbx}
: ${SslDir:=${ConfDir}/ssl}
: ${AuthoritiesDir:=${SslDir}/authorities}
: ${PidFile:=/home/joegen/sipxecs-bin/var/run/sipxpbx/sipxcallresolver.pid}
: ${LibOdbcDir:=/usr/lib}
: ${Psql:=psql}
if [ `uname -s` = FreeBSD ] ; then
    : ${ServiceDir:=/home/joegen/sipxecs-bin/etc/rc.d}
else
    : ${ServiceDir:=/etc/init.d}
fi
: ${SubstituteUser:=su}
: ${Chown:=chown}
: ${LogDir:=/home/joegen/sipxecs-bin/var/log/sipxpbx}
: ${TmpDir:=/home/joegen/sipxecs-bin/var/sipxdata/tmp}
: ${RunningUser:=joegen}
: ${PgUser:=postgres}
: ${PgSchema:=${ConfDir}/cdr/schema.sql}
: ${RubyCmd:=/usr/bin/ruby}
: ${DbVersion:=7}
: ${DbPatchDir:=${ConfDir}/cdr}
: ${DbPatchLog:=${LogDir}/sipcdrpatch.log}
: ${CallResolverLog:=${LogDir}/sipcallresolver.log}
: ${CallResolverMain:=/home/joegen/sipxecs-bin/lib/ruby/gems/1.8/gems/sipxcallresolver-2.0.0/lib/main.rb}
...
configCheck() {
   ...
   if ! cat /home/joegen/sipxecs-bin/etc/sipxpbx/sipXproxy-config | grep SIPX_PROXY_CALL_STATE_DB | grep ENABLE >/dev/null ; then
      cat <<ERROR
...
ERROR
   ...
   if [ ${Action} = CONFIGTEST ]; then
      ...
      if ! sipx_config_exists /home/joegen/sipxecs-bin/etc/sipxpbx/callresolver-config ; then
          resolveOk=0
          Status=1
      else
          resolveOk=1

          resolveConfig=`sipx_config_value  /home/joegen/sipxecs-bin/etc/sipxpbx/callresolver-config SIP_CALLRESOLVER_PURGE`
  ...
  to /home/joegen/sipxecs-bin/etc/sipxpbx/sipxcallresolver/callresolver-config.vm
...
removeOldCronEntry()
{
    ...
    if [ -f /home/joegen/sipxecs-bin/etc/crontab ]; then
       if grep sipxcallresolver /home/joegen/sipxecs-bin/etc/crontab >/dev/null; then
          sed -e 's/^.*sipxcallresolver.*$//' /home/joegen/sipxecs-bin/etc/crontab > /home/joegen/sipxecs-bin/etc/crontab.tmp
          mv /home/joegen/sipxecs-bin/etc/crontab.tmp /home/joegen/sipxecs-bin/etc/crontab
       fi
    fi
}
...
onSetup() {
    ...
    /home/joegen/sipxecs-bin/bin/pgpatch.sh
...
if [ ${Action} = CONFIGTEST ]; then
    ...
    sipx_stop sipxcallresolver /home/joegen/sipxecs-bin/var/run/sipxpbx/sipxcallresolver.pid
