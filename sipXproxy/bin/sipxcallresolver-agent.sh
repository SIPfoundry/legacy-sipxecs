...
CONFIG_FILE="/home/joegen/sipxecs-bin/etc/sipxpbx/sipxcallresolver-agent-config"
...
case ${Action} in
   RUN)
     ...
     exec /usr/bin/stunnel "${CONFIG_FILE}"
     ;;

   STOP)
     # the stunnel command creates the pid file as specified by the configuration file
     MYPID=`cat /home/joegen/sipxecs-bin/var/run/sipxpbx/sipxcallresolver-agent.pid 2>/dev/null`
     ...
     sipx_stop sipxcallresolver-agent /home/joegen/sipxecs-bin/var/run/sipxpbx/sipxcallresolver-agent.pid

     ;;

   CONFIGTEST)
     # Check that the log file is writable.
     logfile="/home/joegen/sipxecs-bin/var/log/sipxpbx/sipxcallresolver-agent.log"
