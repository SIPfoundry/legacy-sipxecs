  ...
  SipBidirectionalProcessorPlugin(instanceName, priority)
{
  ...
  {
    MongoDB::ConnectionInfo info(MongoDB::ConnectionInfo::connectionStringFromFile(), GatewayDestDB::NS);
    _gatewayDestDB = new GatewayDestDB(info);
  }

  {
    MongoDB::ConnectionInfo info(MongoDB::ConnectionInfo::connectionStringFromFile(), EntityDB::NS);
    _entityDB = new EntityDB(info);
  }
}
