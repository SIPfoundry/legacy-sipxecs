...
#define SBPUBLIC extern "C"

#include "SBHTEvtLst.h"				 /* Implemented here */

#ifdef WIN32
/* Suppress/disable warnings */
#pragma warning(4 : 4390) /* empty controlled statement found
			     (from Libwww macros) */
...
PRIVATE void EventList_dump (void)
{
	...
	cur = HashTable[v];
	while ((pres = (SockEvents *) HTList_nextObject(cur)) != NULL) {
	    int i;
...
{
    unsigned long id;
    HWND hwnd = SBinetHTEventList_getWinHandle(&id);
...
// PRIVATE void CheckSockEvent (HTTimer * timer, HTTimerCallback * cbf, void * param)
// {
//     SockEvents * sockp = (SockEvents *)param;
...
PRIVATE SockEvents * SockEvents_get (SOCKET s, SockEvents_action action)
{
    ...
    if (action == SockEvents_mayCreate) {
	    ...
	    HT_OUTOFMEM((char*)"SBinetHTEventList_register");
	pres->s = s;
...
PRIVATE int EventOrder_add (SOCKET s, HTEventType type, ms_t now)
{
	...
	HTTRACE(THD_TRACE, "EventOrder.. no event found for socket %d, type %s.\n" _
      ...
      // Don't abort the whole IO thread for one transient condition. 
      // Hopefully, this socket will be unregistered soon anyway.
	  return HT_OK;
    }
	...
	if (pres->s == s && pres->event == event && pres->type == type) {
	    // tmp is required to eliminate Microsoft compiler error since
	    // this increments an enum
	    int tmp = ((int) pres->skipped) + 1;
	    pres->skipped = (HTPriority) tmp;
	    return HT_OK;
	}
	...
	HT_OUTOFMEM((char*)"EventOrder_add");
    pres->event = event;
    pres->s = s;
    pres->type = type;
...
PRIVATE int EventOrder_executeAndDelete (void) 
{
    HTList * cur = EventOrderList;
    EventOrder * pres;
    int i = 0;
    ...
    while (((pres=(EventOrder *) HTList_removeLastObject(cur)) != NULL) && 
	   (i<EVENTS_TO_EXECUTE)) {
	HTEvent * event;
	int ret;
	event = pres->event;
...
PRIVATE BOOL EventOrder_clearAll (void)
{
    ...
    if (cur) {
        while ((pres = (EventOrder *) HTList_nextObject(cur)) != NULL)
	    HT_FREE(pres);
	return YES;
    }
    return NO;
...
PRIVATE BOOL EventOrder_deleteAll (void) 
{
    EventOrder_clearAll();
...
*/
int SBinetHTEventList_register (SOCKET s, HTEventType type, HTEvent * event)
{
    int newset = 0;
    SockEvents * sockp;
    ...
    */
    ...
    HTTRACEDATA((char *) FdArray+HTEvent_INDEX(type), 8, "SBinetHTEventList_register: (s:%d)" _ s);
...
*/
int SBinetHTEventList_unregister (SOCKET s, HTEventType type) 
{
    long 		v = HASH(s);
    ...
    while (cur && (pres = (SockEvents *) HTList_nextObject(cur)) != NULL) {
        if (pres->s == s) {
	    int	remaining = 0;
	    ...
	    */
	    ...
	    HTTRACEDATA((char*)FdArray+HTEvent_INDEX(type), 8, "SBinetHTEventList_unregister: (s:%d)" _ s);
...
*/
int SBinetHTEventList_unregisterAll (void) 
{
    int i;
...
*/
int SBinetHTEventList_dispatch (SOCKET s, HTEventType type, ms_t now)
{
    SockEvents * sockp = SockEvents_get(s, SockEvents_find);
    if (sockp) {
...
SBPUBLIC HTEvent * SBinetHTEventList_lookup (SOCKET s, HTEventType type)
{
...
/*
 * Modifed version of event loop for SBinet, includes locking
 */
...
*/
SBPUBLIC void SBinetHTEventList_stopLoop (void)
{
    HTEndLoop = 1;
}
...
*/
SBPUBLIC int SBinetHTEventList_newLoop (void)
{
  /*
  printf("Thread %x entering event loop\n",VXItrdThreadGetID());
  */
#ifdef WWW_WIN_ASYNC
    ...
    /*
     * New SBinet Event loop with locking
     */
    while (!HTEndLoop && GetMessage(&msg,0,0,0)) {
      //if(msg.lParam > 1024) break; // See SPR 7850 for reason

      SBinetInterface::LockLibwww( );
      TranslateMessage(&msg);
      DispatchMessage(&msg);
      // Loop over rest of messages in queue before giving up Inet
      while (!HTEndLoop && PeekMessage(&msg,0,0,0,PM_REMOVE)) {
	    //if(msg.lParam > 1024) break; // See SPR 7850 for reason
	    TranslateMessage(&msg);
	    DispatchMessage(&msg);
      }
	...
	*/
	...
	if ((status = HTTimer_next(&timeout)))
	    break;
	/*
	 * This is a hack, we really should check the fd_sets for emptiness and use a trdTimer
	 *   Requesting threads could wake the timer -dg 6/7/01
	 */
	if(timeout == 0)
	  timeout = 100;
	if (timeout != 0) {
	...
	*/
	...
	fd_dump(maxfds, &treadset, &twriteset, &texceptset, wt);
		...
		*/
		HTTRACE(THD_TRACE, "Event Loop.. select was interrupted - try again\n");
		continue;
	    }
#endif /* EINTR */
#ifdef EBADF
...
PRIVATE LRESULT CALLBACK SBinetAsyncWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    WORD event;
    SOCKET sock;
    HTEventType type;
    ...
    if (SBinetHTEventList_dispatch((int)sock, type, now) != HT_OK)
	HTEndLoop = -1;
    return (0);
}
...
*/
SBPUBLIC BOOL 
SBinetHTEventList_setWinHandle (HWND window, unsigned long message)
{
    HTSocketWin = window;
    HTwinMsg = message;
    return YES;
...
SBPUBLIC HWND SBinetHTEventList_getWinHandle (unsigned long * pMessage)
{
    if (pMessage)
        *pMessage = HTwinMsg;
    return (HTSocketWin);
...
SBPUBLIC BOOL SBinetHTEventInit (void)
{
#ifdef WWW_WIN_ASYNC
    /*
    ...
    */
    ...
    wc.style=0;
    wc.lpfnWndProc=(WNDPROC)SBinetAsyncWindowProc;
    wc.cbClsExtra=0;
    wc.cbWndExtra=0;
    wc.hIcon=0;
    wc.hCursor=0;
    ...
    if ((HTSocketWin = CreateWindow(className, "WWW_WIN_ASYNC", WS_POPUP, CW_USEDEFAULT, CW_USEDEFAULT, 
                                     CW_USEDEFAULT, CW_USEDEFAULT, 0, 0, wc.hInstance,0)) == NULL) {
#ifdef HTDEBUG
	char space[50];
    ...
    */
	...
	if (WSAStartup(DESIRED_WINSOCK_VERSION, &wsadata)) {
	    HTTRACE(THD_TRACE, "SBinetHTEventInit. Can't initialize WinSock\n");
            WSACleanup();
            return NO;
        }
        if (wsadata.wVersion < MINIMUM_WINSOCK_VERSION) {
            HTTRACE(THD_TRACE, "SBinetHTEventInit. Bad version of WinSock\n");
            WSACleanup();
            return NO;
        }
	HTTRACE(APP_TRACE, "SBinetHTEventInit. Using WinSoc version \"%s\".\n" _ 
		    wsadata.szDescription);
    }
#endif /* _WINSOCKAPI_ */

    HTEvent_setRegisterCallback(SBinetHTEventList_register);
    HTEvent_setUnregisterCallback(SBinetHTEventList_unregister);
    return YES;
}

SBPUBLIC BOOL SBinetHTEventTerminate (void)
{
#ifdef _WINSOCKAPI_
    WSACleanup();
#endif /* _WINSOCKAPI_ */
