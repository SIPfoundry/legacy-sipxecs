...
################################################################
## This script is used to copy certificate and key files from
## the WorkDir where they are created by the gen-ssl-keys.sh
## script to the appropriate configuration directories.
##
...
InstallPrefix=""
InstallDir="/home/joegen/sipxecs-bin/etc/sipxpbx"
...
do
    case ${1} in
       ...
       -iq|--install-qualifier)
          #
          # Install qualifier
          #
         if [ $# -lt 2 ]
         then
             echo "Must specify <install-qualifier> with ${1}" 1>&2
              Action=USAGE
              break
          else
                installQualifier="-${2}"
...
if [ -z "${Basename}" ]
then
    Basename="`hostname --fqdn`${installQualifier}"
else
...
then
  ...
  is now integrated into the sipxecs-setup script, supported
...
then
    cat <<EOF 1>&2
! Certificate and key not found for ${WorkDir}/${Basename}
  ${WorkDir}/${Basename}.crt
  ${WorkDir}/${Basename}.key
...
Usage:
   install-cert.sh [ {-wd|--workdir} <working-directory> ]
                   ...
                   [ {-iq|--install-qualifier} <install-qualifier> ]
                   [ <server-name> ]
   ...
   --install-qualifier Specifies the type of key. Can be web or sip
		     or blank (unpsecified). If blank, a the xml-rpc
		     key ( used for https ) is the same as the web key.
...
#Note:
#The CA for the web certificate is not available here because it is installed on the web browser.
#SipxConfig does not install the CA for the web crt and hence we do not verify it.

if  /home/joegen/sipxecs-bin/bin/ssl-cert/check-cert.sh ${AuthoritySwitch} "${AuthorityTarget}" "$WorkDir/$Basename.crt"
then
	...
	# remove whitespaces
	installedCaName=$( echo "${caName}" | sed "s/ //g" )

        cp -v "${caName}.crt" "${InstallPrefix}${InstallDir}/ssl/authorities/${installedCaName}.crt" || exit 1
        chown ${sipXuser}     "${InstallPrefix}${InstallDir}/ssl/authorities/${installedCaName}.crt" || exit 1
        chmod 0644            "${InstallPrefix}${InstallDir}/ssl/authorities/${installedCaName}.crt" || exit 1

        # regenerate the hash symlinks for the certificate store
        /home/joegen/sipxecs-bin/bin/ssl-cert/ca_rehash "${InstallPrefix}${InstallDir}/ssl/authorities"
	...
	#The web key is initially the same as the ssl key.
	if [ "${installQualifier}" = "" ]
	then
		echo "Generating web cert automatically"
                cp -v "$Basename.crt" "${InstallPrefix}${InstallDir}/ssl/ssl-web.crt" || exit 1
                chown ${sipXuser}     "${InstallPrefix}${InstallDir}/ssl/ssl-web.crt" || exit 1
                chmod 0600            "${InstallPrefix}${InstallDir}/ssl/ssl-web.crt" || exit 1

                echo "Installing the '${Basename}' private key"
                cp -v "$Basename.key" "${InstallPrefix}${InstallDir}/ssl/ssl-web.key" || exit 1
                chown ${sipXuser}     "${InstallPrefix}${InstallDir}/ssl/ssl-web.key" || exit 1
                chmod 0600            "${InstallPrefix}${InstallDir}/ssl/ssl-web.key" || exit 1
        ...
        then
            ...
            /home/joegen/sipxecs-bin/bin/ssl-cert/check-cert.sh "${InstallDir}/ssl/ssl${installQualifier}.crt"
            certOk+=$?
        fi
    fi
