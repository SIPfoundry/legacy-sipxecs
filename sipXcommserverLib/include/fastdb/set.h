...
class dbSet { 
  ...
  public:
    ...
    dbSet(T const elem) : bits((set_t)1 << elem) {}
    dbSet(dbSet const& s) : bits(s.bits) {}

    dbSet(T const from, T const till)  {
	assert(from <= till);
    ...
    dbSet operator + (T const elem) const {
    ...
    dbQueryExpression has(char const* field) const {
	dbQueryExpression expr;
