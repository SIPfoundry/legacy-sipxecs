...
// Static Initializers
PublisherDB* PublisherDB::spInstance = NULL;
OsMutex         PublisherDB::sLockMutex (OsMutex::Q_FIFO);
const UtlString PublisherDB::gComponentKey("component");
const UtlString PublisherDB::gUriKey("uri");
const UtlString PublisherDB::gCallidKey("callid");
const UtlString PublisherDB::gContactKey("contact");
const UtlString PublisherDB::gNotifycseqKey("notifycseq");
const UtlString PublisherDB::gSubscribecseqKey("subscribecseq");
const UtlString PublisherDB::gExpiresKey("expires");
const UtlString PublisherDB::gEventtypekeyKey("eventtypekey"); // sic
const UtlString PublisherDB::gEventtypeKey("eventtype");
const UtlString PublisherDB::gIdKey("id");
const UtlString PublisherDB::gToKey("toUri");
const UtlString PublisherDB::gFromKey("fromUri");
const UtlString PublisherDB::gFileKey("file");
const UtlString PublisherDB::gKeyKey("key");
const UtlString PublisherDB::gRecordrouteKey("recordroute");
const UtlString PublisherDB::gAcceptKey("accept");
const UtlString PublisherDB::gVersionKey("version");
const UtlString PublisherDB::sComponentStatus("status");
const UtlString PublisherDB::sAcceptSimpleMessage("application/simple-message-summary");

const UtlString PublisherDB::nullString("");

// The 'type' attribute of the top-level 'items' element.
const UtlString PublisherDB::sType("subscription");

// The XML namespace of the top-level 'items' element.
const UtlString PublisherDB::sXmlNamespace("http://www.sipfoundry.org/sipX/schema/xml/subscription-00-00");
...
{
    OsSysLog::add(FAC_DB, PRI_DEBUG, "<><>## PublisherDB:: DESTRUCTOR");
}
...
{
    OsSysLog::add(FAC_DB, PRI_DEBUG, "<><>## PublisherDB::releaseInstance() spInstance=%p", spInstance);
...
UtlBoolean
PublisherDB::insertRow (const UtlHashMap& nvPairs)
{
...
#       ifdef TEST_DEBUG
        OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::insertRow <<<<<<<<<<<< query:\n"
                      "  component='%s'\n"
                      "  toUri='%s'\n"
                      "  fromUri='%s'\n"
                      "  callid='%s'\n"
...
#                   endif
            ...
            OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::insertRow found %d rows"
#                         ifdef TEST_DEBUG
                          ": %s"
#                         endif
                          ,existing
...
#           ifdef TEST_DEBUG
            OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::insertRow <<<<<<<<<<<< added:\n"
                          "  component='%s'\n"
                          "  to='%s'\n"
                          "  from='%s'\n"
                          "  callid='%s'\n"
...
#       ifdef TEST_DEBUG
        OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::removeRow <<<<<<<<<<<< query:\n"
                      "  toUri='%s'\n"
                      "  fromUri='%s'\n"
                      "  callid='%s'\n"
                      "  subscribecseq='%d'",
...
#       endif
        ...
        {
           OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::removeRow row not found:\n"
   ...
   const UtlString& callid )
{
   if ( m_pFastDB != NULL )
   {
      ...
      if (cursor.select(query) > 0)
      {
         ...
         OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::removeErrorRow rows: %s",
                       errorRows.data()
                       );
         cursor.removeAllSelected();
      ...
      {
         OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::removeErrorRow row not found:\n"
...
UtlBoolean
PublisherDB::subscriptionExists (
   const UtlString& component,
   const UtlString& to,
   const UtlString& from,
   const UtlString& callid,
...
void
PublisherDB::removeExpired( const UtlString& component,
                               const int timeNow )
{
   if (m_pFastDB != NULL)
   {
...
void
PublisherDB::getAllRows ( ResultSet& rResultSet ) const
{
    // Clear the results
...
void
PublisherDB::updateNotifyUnexpiredSubscription (
        const UtlString& component,
        const UtlString& to,
        const UtlString& from,
        const UtlString& callid,
...
#       ifdef TEST_DEBUG
        OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::updateNotifyUnexpiredSubscription <<<<<<<<<<<< query:\n"
                      "  toUri='%s'\n"
                      "  callid='%s'\n"
                      "  eventtypekey='%s'\n"
                      "  id='%s'",
        ...
        if ( cursor.select(query) > 0 )
        {
            do {
                ...
                if ( cursor->expires < timeNow )
                {
                    ...
                    cursor.remove();
                    OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::updateNotifyUnexpiredSubscription removing expired row.");
                } else
                {
                   ...
                   OsSysLog::add(FAC_DB, PRI_DEBUG,
                                 "PublisherDB::updateNotifyUnexpiredSubscription updating notifycseq for '%s' to: %d",
...
void
PublisherDB::getUnexpiredContactsFieldsContaining (
   ...
   UtlSList& matchingContactFields ) const
{
   ...
   {
      OsSysLog::add(FAC_DB, PRI_CRIT, "PublisherDB::getUnexpiredContactsFieldsContaining failed - no DB");
   }
}

UtlBoolean
PublisherDB::updateSubscribeUnexpiredSubscription (
        const UtlString& component,
        const UtlString& to,
        const UtlString& from,
        const UtlString& callid,
...
#       ifdef TEST_DEBUG
        OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::updateSubscribeUnexpiredSubscription <<<<<<<<<<<< query:\n"
                      "  toUri='%s'\n"
                      "  fromUri='%s'\n"
                      "  callid='%s'\n"
                      "  eventtypekey='%s'\n"
...
void
PublisherDB::getUnexpiredSubscriptions (
    const UtlString& component,
    const UtlString& key,
    const UtlString& eventTypeKey,
    const int& timeNow,
   ...
   ) const
{
   ...
   OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::updateToTag callid = '%s', fromtag = '%s', totag = '%s'",
   ...
   if ( m_pFastDB != NULL )
   {
      ...
      if ( cursor.select(query) > 0 )
      {
         do {
            ...
            OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::updateToTag cursor->fromUri = '%s', cursor->toUri = '%s', seen_tag = '%s'",
            ...
            if (r && seen_tag.compareTo(fromtag) == 0)
            {
               ...
               if (!toUri.getFieldParameter("tag", dummy))
               {
                  ...
                  OsSysLog::add(FAC_DB, PRI_DEBUG, "PublisherDB::updateToTag cursor->toUri = '%s'",
                                dummy.data());
...
UtlBoolean PublisherDB::findFromAndTo(
   const UtlString& callid,
   const UtlString& fromtag,
   const UtlString& totag,
   UtlString& from,
...
void PublisherDB::removeExpiredInternal( const UtlString& component,
                                            const int timeNow )
{
   ...
   {
      OsSysLog::add(FAC_DB, PRI_DEBUG,
                    "PublisherDB::removeExpiredInternal removing %d expired subscriptions",
                    expiring
                    );
...
PublisherDB*
PublisherDB::getInstance( const UtlString& name )
{
    // Critical Section here
    OsLock lock( sLockMutex );
