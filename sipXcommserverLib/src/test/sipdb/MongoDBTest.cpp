...
public:
    ...
    BaseDBTest() :
        _info(MongoDB::ConnectionInfo(mongo::ConnectionString(mongo::HostAndPort("127.0.0.1")))),
        _databaseName("test.BaseDBTest")
    {
    }
    ...
    void testForEach()
    {
        ...
        pConn->get()->insert(_databaseName, BSON("a" << 0));
        pConn->get()->insert(_databaseName, BSON("a" << 10));
        pConn->done();
