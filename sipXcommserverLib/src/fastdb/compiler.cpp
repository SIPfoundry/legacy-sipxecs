...
void* dbExprNode::operator new(size_t EXTRA_DEBUG_NEW_PARAMS) 
{ 
...
dbCompiler::dbCompiler() {
    ...
    for (unsigned i = 0; i < itemsof(keywords); i++) { 
        dbSymbolTable::add((char*&)keywords[i].name, keywords[i].tag, FASTDB_CLONE_ANY_IDENTIFIER);    
    }
    initialized = true;
    }
}
...
{
#ifdef __BORLANDC__ 
    // malloc from Borland runtime library returns value aligned only on 4
    // And GigaBASE needs alignment on maximal size of scalar field = 8
    byte* p = (byte*)malloc(size+8);
    byte* q = (byte*)(DOALIGN((long)p + 4, 8));
    *((int*)q-1) = q - p;
    return q;
#else
