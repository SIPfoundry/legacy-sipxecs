...
ReproGlue::ReproGlue(const std::string& applicationName, const std::string& databaseDir) :
  ...
  _sipAuthDisabled(false),
  _pAuthRequestDispatcher(0)
{
...
void ReproGlue::makeRequestProcessorChain(ProcessorChain& chain)
{
   ...
   if (!_sipAuthDisabled && _pAuthRequestDispatcher)
   {
     _pDigestAuthenticator = new RequestProcessor(new DigestAuthenticator(*mProxyConfig, _pAuthRequestDispatcher), *this, "DigestAuthenticator");
   ...
   addProcessor(chain, std::auto_ptr<Processor>(new LocationServer(*mProxyConfig, *mRegistrationPersistenceManager, _pAuthRequestDispatcher)));
...
void ReproGlue::createDialogUsageManager()
{
   ...
   _sipAuthDisabled = mProxyConfig->getConfigBool("DisableAuth", false);
   ...
   if (mDum)
   {
      ...
      if(_sipAuthDisabled && !wsCookieAuthSharedSecret.empty())
      {
      ...
      if (!_sipAuthDisabled)
      {
         ...
         assert(!_pAuthRequestDispatcher);
         ...
         if (_pExternalAuthGrabber)
         {
            ...
            _pAuthRequestDispatcher = new Dispatcher(grabber, mSipStack, numAuthGrabberWorkerThreads);
         }
         else
         {
