...
/**
 * Holds the valid user data needed for the AutoAttendant, parsing from mongo db
 * imdb
 * 
 */
public enum ValidUsers {
	...
	public List<User> getUsers() {
		...
		try {
			DBCursor cursor = getEntityCollection().find(
					QueryBuilder.start(IMDB_VALID).is(Boolean.TRUE).get());
	...
	public User getUser(String userName) {
		DBObject queryUserName = QueryBuilder.start(IMDB_VALID).is(Boolean.TRUE).and(IMDB_UID)
				.is(userName).get();
	 ...
	 * 
	 * @param digits
	 *            DTMF digits to match against user directory
	 * @param onlyVoicemailUsers
	 *            limit match to users in directory who have voicemail
	...
	private static User extractValidUser(DBObject obj) {
		...
		if (permissions != null) {
			...
			user.setCanRecordPrompts(permissions
					.contains(IMDB_PERM_RECPROMPTS));
		...
		user.setAttachAudioToEmail(Boolean.valueOf(getStringValue(obj,
				IMDB_ATTACH_AUDIO)));
		...
		user.setAltAttachAudioToEmail(Boolean.valueOf(getStringValue(obj,
		        IMDB_ALT_ATTACH_AUDIO)));
		...
		if (obj.keySet().contains(IMDB_SYNC)) {
			...
			imapInfo.setSynchronize(Boolean
					.valueOf(getStringValue(obj, IMDB_SYNC)));
			...
			if (imapInfo.getAccount() == null
					|| imapInfo.getAccount().length() == 0) {
	...
	/**
	 * Remove all non-letter characters, convert to upper case Remove
