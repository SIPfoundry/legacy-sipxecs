...
static struct myregexp_proc *get_regexp_proc_instance(struct myregexp_proc *, oid);
struct myregexp_proc  *regexp_procwatch = NULL;
...
void
regexp_proc_free_config(void)
{
    struct myregexp_proc  *ptmp, *ptmp2;

    for (ptmp = regexp_procwatch; ptmp != NULL;) {
        ptmp2 = ptmp;
        ptmp = ptmp->next;
        if (ptmp->regexp) {
            free(ptmp->regexp);
 ...
 */
static struct myregexp_proc *
get_regexp_proc_by_name(char *name)
{
    struct myregexp_proc  *ptmp;

    if (name == NULL)
        return NULL;

    for (ptmp = regexp_procwatch; ptmp != NULL && strcmp(ptmp->name, name) != 0;
         ptmp = ptmp->next);
    return ptmp;
}
             ...
             u_char * statP, oid * name, size_t name_len)
{
    ...
    if ((proc = get_regexp_proc_instance(regexp_procwatch, name[10]))) {
...
static struct myregexp_proc *
get_regexp_proc_instance(struct myregexp_proc *proc, oid inst)
{
    int             i;
